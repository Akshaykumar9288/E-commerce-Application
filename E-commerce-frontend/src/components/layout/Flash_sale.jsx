...
useEffect(() => {
    ...
    <div
        key={product._id}
        className="product-card px-[16px] py-[16px] min-w-[280px] h-full flex flex-col gap-[4px] items-start bg-[#fff] rounded-[12px]"
    >
        <div
            className="product-image h-[192px] w-full flex justify-center items-center rounded-[8px] bg-[#F8F9FA] relative"
            style={{
        ...
        >
            <div className="percent-off absolute top-[8px] left-[8px] bg-[#BA1A1A] w-[61px] h-[23px] rounded-[4px] py-[4px] px-[8px] text-white text-[10px] font-bold leading-[15px]">
                <p>{product.discount}% OFF</p>
            </div>
        </div>

        <div className="product-name w-full h-[36px] pt-[12px]">
            <p className="text-[#191C1D] leading-[24px]">
                {product.title}
            </p>
        </div>

        <div className="product-price h-[24px] w-full flex gap-[8px] items-center">
            <p className="text-[#8A5100] font-bold leading-[24px]">
                ₹{product.price}
            </p>

            <p className="text-[#554434] text-sm leading-[24px] line-through">
                ₹{product.originalPrice}
            </p>
        </div>

        <div className="add-to-card-button pt-[20px] pb-[8px] h-[52px] w-full flex justify-center items-center">
            <a href="#!" className="text-[#0061A3] text-center leading-[24px]">
                Add to Cart
            </a>
        </div>
    </div>
